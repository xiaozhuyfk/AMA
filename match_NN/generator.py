...
class ClickDataGenerator(Configurable):
    ...
    max_title_len = Int(50, help='max title len').tag(config=True)
    q_name = Unicode('q')
    d_name = Unicode('d')
    aux_d_name = Unicode('d_aux')
    idf_name = Unicode('idf')
    neg_sample = Int(5, help='negative sample').tag(config=True)
    min_score_diff = Float(0.1, help='min score difference for click model generated pairs').tag(config=True)
    ...
    def pointwise_generate(self, pair_stream, batch_size, with_label=True, with_idf=False):
        ...
        for line in pair_stream:
            cols = line.split('\t')
            q = np.loadtxt(StringIO(unicode(cols[0])), delimiter=',',  dtype=int,)
            doc = np.loadtxt(StringIO(unicode(cols[1])), delimiter=',',  dtype=int,)
            if with_idf:
                idf = np.loadtxt(StringIO(unicode(cols[2])), delimiter=',',  dtype=float)
            y = 0
            if with_label:
                y = int(cols[-1])
            v_q = np.ones(self.max_q_len) * -1
            v_d = np.ones(self.max_title_len) * -1
            v_q[:q.shape[0]] = q[0:min(q.shape[0], self.max_q_len)]
            v_d[:doc.shape[0]] = doc[0:min(doc.shape[0], self.max_title_len)]
    ...
    def pairwise_reader(self, pair_stream, batch_size, with_idf=False):
        ...
        for line in pair_stream:
            ...
            t1 = np.array([int(t) for t in cols[1].split(',')])  #np.loadtxt(StringIO(unicode(cols[1])), delimiter=',',  dtype=int,)
            t2 = np.array([int(t) for t in cols[2].split(',')])  #np.loadtxt(StringIO(unicode(cols[2])), delimiter=',',  dtype=int,)
            y = float(cols[3])
            ...
            if y > 0:
                ...
                y = 1
            if with_idf:
                idf = np.array([float(t) for t in cols[4].split(',')])  #np.loadtxt(StringIO(unicode(cols[4])), delimiter=',',  dtype=float)
            v_q = np.ones(self.max_q_len) * -1
            v_d = np.ones(self.max_title_len) * -1
            v_d_aux = np.ones(self.max_title_len) * -1
            v_q[:min(q.shape[0], self.max_q_len)] = q[:min(q.shape[0], self.max_q_len)]
            v_d[:min(t1.shape[0], self.max_title_len)] = t1[:min(t1.shape[0], self.max_title_len)]
            v_d_aux[:min(t2.shape[0], self.max_title_len)] = t2[:min(t2.shape[0], self.max_title_len)]
